...
- T8 corrige el reflow extremo detectado al cerrar la spec: el indicador de etapas puede envolver sin conector a 30rem o menos; las columnas del menú ceden ancho, su acción reduce padding horizontal, el logo móvil queda separado del disparador del panel y la autoría del pie puede envolver. `DESIGN.md` documenta estos ajustes. Una prueba adicional confirma que el reproductor no crea audio con movimiento reducido.
- T7 verifica el límite de persistencia sin cambios de producto: `quizStore` solo conserva la ronda actual en `progress` para reanudar o revisar y `bestResults` como resumen duradero. Al volver al menú, `round` y `answers` quedan vacíos. `preferencesStore` solo guarda `theme` y `soundEnabled`; el panel deriva la matriz visible de marcas y no crea un modelo de partidas o intentos. Se añade una prueba del contrato serializado tras una ronda completada y el regreso al menú.
- T6 incorpora los cuatro MP3 locales del pack `soft` de UI SFX (success, error, warning y complete; CC0) sin dependencias de runtime. El reproductor crea `HTMLAudioElement` solo después de una activación deliberada y usa un volumen fijo de `0.25`; una nueva señal detiene la anterior y el silencio detiene cualquier reproducción en curso. No reproduce audio cuando la pestaña está oculta o se solicita movimiento reducido. La respuesta correcta, la incorrecta y el tiempo agotado se asocian a transiciones efectivas del store; `complete` suena solo al finalizar una partida configurada, nunca al terminar una repetición de fallos ni al restaurar resultados.
- El panel muestra «Efectos sonoros» como checkbox nativo, con iconos de altavoz activo y silenciado, foco visible y texto de estado. El icono es decorativo; el nombre y el estado del checkbox siguen disponibles para tecnologías de asistencia. El usuario pidió este ajuste visual durante T6. Las bandas de respuesta, el aviso de tiempo agotado y el resumen final conservan el feedback textual existente.
- Refinamiento visual de T6 solicitado por el usuario: «Efectos sonoros» adopta el peso del rótulo «Tema»; debajo, el icono queda a la izquierda y la explicación a la derecha. «Activados» y «Desactivados» se destacan en negrita, con el token `progress` solo para el estado activo. El checkbox mantiene un nombre accesible estable.
...
- 2026-09-23, cierre de spec 006 — El usuario confirmó que los sonidos están bien y dio por cerrada la spec. Esta revisión resuelve la reserva de valoración auditiva subjetiva registrada en T8; no hubo cambios de código posteriores a los checks completos.
- 2026-09-23, T8 — Checks run: `pnpm format:check`, `pnpm typecheck`, `pnpm lint`, `pnpm test` (40 archivos, 228 pruebas), `pnpm build` y `git diff --check`; todos pasan. Las pruebas cubren el flujo completo por teclado, migración v1→v2, contratos inválidos, persistencia separada, marcas por modalidad, foco del panel, bootstrap y persistencia del tema, bloqueo inicial y silencio del audio, transiciones sonoras y movimiento reducido. En Chromium se revisó el panel a 320 × 700 CSS px en claro y oscuro, `forced-colors` con foco visible, texto al 200 % tanto a 640 como a 320 px, cierre y restauración de foco, y movimiento reducido (animaciones computadas de 0 s). A 320 px, incluso con el texto al 200 %, documento y diálogo miden 320 px de ancho, sin scroll horizontal; el logo y el disparador tienen separación. El viewport de 320 CSS px se usó como equivalente de reflow de 1280 px al 400 % de zoom, sin automatizar el control de zoom del navegador. Los cuatro MP3 distintos duran 0,76–0,98 s, ocupan unos 36 KB en conjunto y tienen RMS de −18,8 a −20,0 dBFS antes del volumen interno `0.25`. Failing checks: ninguno. Unresolved risks: la apreciación auditiva subjetiva de confort y diferencia semántica no se puede certificar con pruebas automáticas; el usuario puede silenciar los efectos.
- 2026-09-23, T7 — Checks run: búsqueda de escrituras y modelos de historial en `src`, revisión de `QuizState`, esquemas v2/v1 y `partializeQuizState`, selector de marcas del panel y transiciones de menú; `pnpm format:check`, `pnpm typecheck`, `pnpm lint`, `pnpm test` (40 archivos, 227 pruebas), `pnpm build` y `git diff --check`. Passing checks: todos pasan; no hay colección de partidas o intentos y, tras salir de resultados, se conserva la marca mientras se vacía la ronda persistida. Failing checks: ninguno. Unresolved risks: las verificaciones generales de cierre pertenecen a T8.
- 2026-09-23, refinamiento visual de T6 — Checks run: `pnpm format:check`, `pnpm typecheck`, `pnpm lint`, `pnpm test` (40 archivos, 226 pruebas), `pnpm build` y `git diff --check`. Passing checks: todos pasan. En Chromium se comprobó la nueva jerarquía, la disposición en fila y los dos estados visuales con el nombre accesible «Efectos sonoros».
- 2026-09-23, T6 — Checks run: `pnpm format:check`, `pnpm typecheck`, `pnpm lint`, `pnpm test` (40 archivos, 226 pruebas), `pnpm build` y `git diff --check`. Passing checks: todos pasan. Las pruebas cubren el bloqueo anterior al gesto, silencio, volumen, sustitución del cue activo, transición única para acierto/error/tiempo agotado, cierre de partida configurada y exclusión de repeticiones; el panel prueba persistencia, nombre del checkbox y ambos iconos. En Chromium se comprobó el cambio visual entre altavoz activo/silenciado, el estado textual, la persistencia tras recarga, el foco del diálogo y el feedback de una respuesta correcta. Vite sirvió `success.mp3` con HTTP 200 y tipo `audio/mpeg`; los cuatro archivos son MP3 mono de 44,1 kHz y ocupan aproximadamente 36 KB en conjunto. Unresolved risks: la valoración auditiva subjetiva de los cuatro cues y la revisión visual a 320 CSS px, zoom y `forced-colors` siguen pendientes para T8.

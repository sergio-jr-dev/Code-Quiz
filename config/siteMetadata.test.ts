...
describe('public site metadata', () => {
  ...
  it.each(['http://example.com', 'https://user:pass@example.com', 'https://example.com/quiz/', 'https://example.com/?a=b', 'https://example.com/#quiz', 'not-a-url'])('rejects invalid origin %s', url => {
  ...
  it('accepts an HTTPS origin with or without the final slash', () => {
    ...
    expect(getSiteMetadata('https://example.com/')).toEqual([
      ...
      { tag: 'meta', attrs: { property: 'og:url', content: 'https://example.com/' }, injectTo: 'head' },
      { tag: 'meta', attrs: { property: 'og:image', content: 'https://example.com/images/og-image.png' }, injectTo: 'head' },
    ]);
  });
});
